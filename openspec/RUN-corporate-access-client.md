...
| 2 | `stage/2-key-only-onboarding` | key-only-onboarding | код готов, замечания разобраны; **CI на HEAD не подтверждён (биллинг Actions)** | #6 |
...
Этап 2, `stage/2-key-only-onboarding`. Цикл замечаний пройден (4 круга) плюс разбор /mr-fix. Задачи 1.1–4.2, 5.1–5.3, 6.2 из `tasks.md` закрыты.

**Не мержить до подтверждения CI.** HEAD MR #6 — `6626a883`. Полный зелёный прогон есть только на `fc9b9866` (35855056999, круг 3). После него в MR попали правки круга 4 (мёртвый код в `home_page.dart`) и /mr-fix (точки в `msg` во всех 11 локалях, границы слова в тесте) — они проверены **только локально** (`flutter analyze` 0 ошибок / 361 info, `flutter test` 32/32).

Прогоны на `52b68947` и `6626a883` — failure, но **не по коду**: job не стартовал, аннотация GitHub «The job was not started because recent account payments have failed or your spending limit needs to be increased». Пока биллинг Actions не восстановлен, CI на финальном коммите подтвердить нельзя, и task 6.1 снова открыт. Осталось: 5.4 (ручная проверка на пользователе); 6.1 закрыт — CI на MR #6 зелёный целиком. MR #6 в `feat/corporate-access-client`, коммит e5b1dd75.
...
- Слияние каждого MR.
- **Биллинг GitHub Actions**: с 2026-09-23 прогоны не стартуют — «recent account payments have failed or your spending limit needs to be increased». Пока не восстановлен, CI на финальных коммитах подтвердить нельзя; MR #6 не мержить до зелёного прогона на HEAD.
