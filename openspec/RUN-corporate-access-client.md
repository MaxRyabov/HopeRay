...
**Цикл замечаний закрыт 2026-09-24 (4 круга, замечаний нет — бот не отозвался), CI зелёный на `c3afb030`. MR #7 ждёт слияния пользователем.**
...
- Этап 3a (после /mr-fix #4103363702): `resetDisabledChainStatus(container, applyToCore:)` сначала отправляет опции ядру (`_applyCoreOptions` в `bootstrap.dart`), настройку пишет в `off` только при успехе — иначе повтор на следующем запуске. Опции ядра от сохранённого значения не зависят: `ConfigOptionRepository` всё равно даёт `off`.
...
| #7 | 4103364010 | таймаут `_safeInit` не отменяет поздний `changeOptions`, гонка с UI | Обе стороны отправляют опции с `off` из тех же настроек; локальный gRPC — миллисекунды, а поздний успех лишь допишет `off` в настройку |
