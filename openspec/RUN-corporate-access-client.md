...
- MR #7, CI на `7ab4d84a` (прогон 36130355362): упала только `android-aab` — Gradle посреди сборки сам докачивал Android SDK Platform 33 для плагина `flutter_displaymode` и получил битый архив («Archive is not a ZIP archive»); `android-apk` на том же коммите зелёная, код ни при чём. По просьбе пользователя исправлено в этом же MR: в `build.yml` отдельный шаг ставит `platforms;android-31` и `android-33` через `sdkmanager` до сборки, с тремя попытками и проверкой `android.jar`. Проверено прогоном 36133734857 на `2715ecd1`: зелёный целиком (`test` и 6 сборок); в логах обеих Android-сборок повторов `sdkmanager` не было, и Gradle больше ничего не докачивал (строк «Installing Android SDK Platform» нет).
