...
### Requirement: Нет автозапуска туннеля после обновления на Android
После обновления приложения на Android туннель MUST NOT подниматься автоматически. Автозапуск после перезагрузки устройства (`ACTION_BOOT_COMPLETED`) MUST сохраниться.

#### Scenario: Обновление при включённом туннеле
- **WHEN** туннель был включён пользователем, и приложение обновляется поверх установки
- **THEN** после обновления туннель не поднимается сам, пока пользователь не включит его

#### Scenario: Перезагрузка устройства
- **WHEN** туннель был включён пользователем, и устройство перезагружается
- **THEN** туннель поднимается автоматически, как раньше
...
- **THEN** профиль не добавляется, пользователь видит локализованную ошибку «Конфигурация не поддерживается: профили Cloudflare WARP недоступны»
...
- **THEN** содержимое профиля не меняется, а уведомление об ошибке обновления называет причину: неподдерживаемая конфигурация WARP
