...
### Requirement: Профили без ключа не принимаются
Приложение MUST NOT добавлять, обновлять и подключать профиль, содержимое которого задаёт выход без выданного ключа: строки со схемами `warp://` или `psiphon://`, конфигурацию с outbound или endpoint типа `warp` или `psiphon`. Это касается содержимого в открытом виде и в base64.

#### Scenario: Вставка warp:// из буфера обмена
- **WHEN** пользователь добавляет из буфера обмена текст `warp://auto#WARP`
- **THEN** профиль не добавляется, пользователь видит локализованную ошибку «Конфигурация не поддерживается: профили Cloudflare WARP и Psiphon недоступны»

#### Scenario: Вставка psiphon:// из буфера обмена
- **WHEN** пользователь добавляет из буфера обмена текст `psiphon://auto/`
- **THEN** профиль не добавляется, пользователь видит ту же ошибку
...
#### Scenario: Активный WARP-профиль, сохранённый до обновления
- **WHEN** приложение запускается, и активный профиль, добавленный до обновления, содержит WARP или Psiphon
- **THEN** профиль перестаёт быть активным, ядру передаётся актуальное состояние
- **AND** при нажатии «Подключиться» показывается диалог «Добавьте ключ доступа» и окно добавления профиля

#### Scenario: Выбор WARP-профиля вручную
- **WHEN** пользователь выбирает активным профиль, содержащий WARP или Psiphon, и нажимает «Подключиться»
- **THEN** подключение не выполняется, пользователь видит ошибку с именем профиля и советом удалить его и добавить выданный ключ

#### Scenario: Смена профиля при включённом туннеле
- **WHEN** туннель включён, и пользователь делает активным профиль, содержащий WARP или Psiphon
- **THEN** переподключение не выполняется, пользователь видит ту же ошибку

#### Scenario: Фоновое обновление WARP-подписки
- **WHEN** подписка при фоновом обновлении начинает содержать WARP
- **THEN** содержимое профиля не меняется, а уведомление об ошибке обновления называет имя профиля и причину: неподдерживаемая конфигурация

#### Scenario: Редактор JSON профиля
- **WHEN** пользователь открывает редактор JSON профиля
- **THEN** шаблоны и автодополнение не предлагают тип `warp`
