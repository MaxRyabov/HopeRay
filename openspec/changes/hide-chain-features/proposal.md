...
- Профили, которые сами по себе дают выход в интернет без выданного ключа, не добавляются и не подключаются: строки `warp://…` и `psiphon://…`, конфиги с outbound/endpoint типа `warp` или `psiphon`. Если при запуске активным оказывается такой профиль, сохранённый до обновления, он перестаёт быть активным: плитка быстрых настроек не поднимет его в фоне, а кнопка подключения ведёт к добавлению ключа.
