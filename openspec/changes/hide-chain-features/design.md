...
- **Проверка содержимого на WARP — чистая функция** `ProfileParser.containsWarp(String content)`. Контент декодируется через `safeDecodeBase64`, ищутся строки со схемой `warp://` и JSON с `"type": "warp"` (outbound или endpoint). Вызов стоит в `ProfileRepositoryImpl.validateConfig`, через который проходят все пути добавления и обновления (`upsertRemote`, `addLocal`, `offlineUpdate`, обновление подписки), до валидации ядром. При срабатывании возвращается новый вариант `ProfileFailure.unsupportedConfig()`. Текст ошибки берётся из перевода в `present(t)`, как у остальных ошибок: у `ProfileRepositoryImpl` нет доступа к переводам. Так же сообщение попадает в тост фонового обновления (`profiles_update_notifier.dart`), и пользователь видит причину. Для профилей, сохранённых до обновления, та же проверка стоит в `ConnectionRepository` перед стартом: читается файл профиля из `profilePathResolver`. Отказ — новый `ConnectionFailure.unsupportedProfile(profileName)`, текст с именем профиля и советом удалить его и добавить выданный ключ.
- **`BootReceiver` не реагирует на `ACTION_MY_PACKAGE_REPLACED`.** Сброс в bootstrap срабатывает только при открытии UI, а `BootReceiver` сразу после обновления вызывает `BoxService.start()` → `Mobile.start` со старыми опциями ядра. Пропуск этого события закрывает окно одной строкой Kotlin. Альтернатива — проверка флага в нативном старте. Отклонена как более сложная; решение продукта — не поднимать туннель после обновления.
- **Редактор JSON (`json_editor.dart`)** при выключенном флаге не предлагает `warp` в шаблонах и автодополнении. Обычный WireGuard к серверам Cloudflare таким способом не распознаётся; это вне scope, пользователь сам вводит ключи.

## Risks / Trade-offs

- [После обновления пользователь на Android должен сам включить VPN] → осознанная цена закрытия окна со старыми опциями; после перезагрузки телефона автостарт работает.
- [Плитка быстрых настроек до первого открытия приложения после обновления поднимет туннель со старыми опциями ядра, в том числе с сохранённым WARP-профилем] → это явное действие пользователя; окно закрывается первым открытием приложения. На iOS аналогично действует on-demand VPN, если его включали. Принято как остаточный риск.
