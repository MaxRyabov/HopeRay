...
- **Удаляемое:**
  ...
  Регистранты плагинов и lock-файлы обновляются через `flutter pub get` на CI. `ios/Podfile.lock` и `macos/Podfile.lock` правятся вручную (удаление записей Sentry); `PODFILE CHECKSUM` считается от Podfile и не меняется. Мака под рукой нет, поэтому корректность подтверждает сборка в CI.
...
- [Ручная правка `Podfile.lock` может не совпасть с тем, что сгенерирует CocoaPods] → iOS- и macOS-сборки в CI запускают `pod install`; расхождение проявится как ошибка или изменение lock, тогда lock берётся с мака.
