...
- **WHEN** по `lib/`, `ios/Runner/`, `macos/Runner/`, `android/app/src/` ищется `sentry` без учёта регистра
- **THEN** совпадений нет
...
- **WHEN** приложение впервые устанавливается и запускается на Android через перехватывающий прокси (mitmproxy/Charles), пользователь проходит интро и открывает окно добавления профиля
