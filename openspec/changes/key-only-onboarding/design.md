...
- **Диалог «нет профиля» с одной кнопкой «OK».** Порядок в `connection_button.dart` уже открывает окно добавления после закрытия диалога. Отдельная кнопка «Добавить ключ» дублировала бы этот переход и открывала бы лист дважды. Текст меняется на «Чтобы подключиться, добавьте ключ доступа, выданный вашей организацией: вставьте его из буфера обмена, отсканируйте QR-код или введите вручную». Заголовок: «Добавьте ключ доступа».
