...
- [ ] 1.1 Удалить `lib/features/profile/add/widgets/free_btns.dart` и `lib/features/profile/add/model/free_profiles_model.dart`
- [ ] 1.2 Удалить `FreeSwitchNotifier`, `FreeProfilesNotifier`, `freeProfilesFilteredByRegion` из `profile_notifier.dart`
- [ ] 1.3 Удалить `FreeProfileConsentDialog` и метод `dialog_notifier.dart`, который его показывает
- [ ] 1.4 Перегенерировать код: `dart run build_runner build --delete-conflicting-outputs`

## 2. Окно добавления профиля

- [ ] 2.1 Удалить `lib/features/profile/add/widgets/nav_bar.dart`
...
- [ ] 3.3 Удалить `lib/features/home/widget/empty_profiles_home_body.dart` и закомментированный вызов в `home_page.dart`

## 4. Строки перевода

- [ ] 4.1 Удалить `common.free`, `pages.profiles.freeSubNotFound`, `pages.profiles.freeSubNotFoundForRegion` и ключи `FreeProfileConsentDialog`; удалить `common.help`, если после правок у него нет использований
- [ ] 4.2 `dart run slang`

## 5. Тесты

- [ ] 5.1 Widget-тест `AddProfileOptions`: нет `Switch` и текста `t.common.free`, нет чипа «Help»; есть кнопки буфера обмена и ручного ввода
...
- [ ] 6.1 `flutter analyze` без новых замечаний, `flutter test` зелёный (в CI)
